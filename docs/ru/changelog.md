# История

---

## 2.1.1

-   📚 Обновлена документация

-   🔨 Почищена типизация

---

## 2.1.0

-   ♻️ Изменен порядок применения функций: omit, defaults, nullish, required, defined, cast, transform, replace, rename, pick, exclude

---

## 2.0.9

-   ➕ Добавлен экспериментальный `nullish`

---

## 2.0.8

-   🐛 Исправлена сортировка ключей

---

## 2.0.7

-   ⬆️ Обновлены зависимости

---

## 2.0.6

-   🚚 Git репозиторий перенесён в другую организацию

---

## 2.0.5

-   ♻️ Все приватные поля теперь общедоступны

---

## 2.0.4

-   ⬆️ Обновлены зависимости

---

## 2.0.3

-   ⬆️ Обновлены зависимости

---

## 2.0.2

-   ⬆️ Обновлены типы

---

## 2.0.1

-   🐛 Исправлены мелкие ошибки

---

## 2.0.0

-   ♻️ Переписан с JavaScript на TypeScript

-   ♻️ Переименовано `TYPES_ENUM` в `CAST_TYPE`

-   ⬆️ Обновлены зависимости

---

## 1.6.1

-   ⬆️ Обновлены зависимости

---

## 1.6.0

-   ➕ Добавлен статический метод `testConfig`

-   ♻️ Произведен рефакторинг исключений. `Vicis.constructor` и `Vicis.config` теперь выбрасывают `AggregateError` вместо `ValidationError`.

---

## 1.5.2

-   ⬆️ Обновлены зависимости

---

## 1.5.1

-   ⬆️ Обновлены зависимости

-   📚 Обновлена документация

---

## 1.5.0

-   ⚡️ Изменён формат экспорта по умолчанию

---

## 1.4.0

-   ➕ Добавлены статические методы `from` и` fromArray`

-   ➕ Добавлена конфигурация из функции

---

## 1.3.12

-   ⬆️ Обновлены зависимости

---

## 1.3.11

-   ⬆️ Обновлены зависимости

---

## 1.3.10

-   ⬆️ Обновлены зависимости

---

## 1.3.9

-   📚 Обновлена документация

---

## 1.3.8

-   🐛 Исправлена ошибка, когда transform принимает класс, а не функцию

---

## 1.3.7

-   ♻️ Рефакторинг трансформации

---

## 1.3.6

-   📚 Обновлена документация

---

## 1.3.5

-   🐛 Исправление ошибок TypeScript

---

## 1.3.4

-   🐛 Исправлена ошибка при пустых данных

---

## 1.3.3

-   ♻️ Рефакторинг внутреннего кеширования

---

## 1.3.2

-   ➖ Удалена зависимость `lodash.clonedeep`

---

## 1.3.1

-   ➖ Убран экспорт по умолчанию

---

## 1.3.0

-   ➕ Добавлен метод `order`

---

## 1.2.0

-   ➕ Добавлен метод `clear`

-   💥 Свойство конфигурации `sort` теперь вседа `false`

-   ♻️ Рефакторинг типа приведения `JSON`

---

## 1.1.3

-   ⬆️ Обновлены зависимости

---

## 1.1.2

-   ⬆️ Обновлены зависимости

-   📚 Обновлена документация

---

## 1.1.1

-   📚 Обновлена документация

---

## 1.1.0

-   ♻️ Рефакторинг метода `data`. Теперь он пытается вызвать методы `toObject` или `toJSON` перед сериализацией.

---

## 1.0.15

-   📚 Обновлена документация

---

## 1.0.14

-   📚 Обновлена документация

---

## 1.0.13

-   ⬆️ Обновлены зависимости

---

## 1.0.12

-   ⬆️ Обновлены зависимости

-   📚 Обновлена документация

---

## 1.0.11

-   📚 Обновлена документация

---

## 1.0.10

-   📚 Обновлена документация

---

## 1.0.9

-   ♻️ Рефакторинг `INTEGER` и `NUMERIC`

---

## 1.0.8

-   📚 Обновлена документация

---

## 1.0.7

-   ➕ Добавлено свойство конфигурации `exclude`

---

## 1.0.6

-   ➕ Новый тип приведения `FLAG`

---

## 1.0.5

-   ♻️ Рефакторинг функции `toString`.

---

## 1.0.4

-   ♻️ Рефакторинг функции `transform`. Теперь аргумент `data` клонируется.
Таким образом, данные не меняются между функциями.

-   📚 Обновлена документация

---

## 1.0.3

-   ♻️ Рефакторинг функции `transform`. Теперь третьим аргументом приходит `data`.

-   ⬆️ Обновлены зависимости

-   📚 Обновлена документация

---

## 1.0.2

-   ➕ Добавлены статические свойства для типов приведения: BOOLEAN, NUMERIC, INTEGER, STRING, JSON

-   ⬆️ Обновлены зависимости

-   📚 Обновлена документация

---

## 1.0.1

-   🔨 Рефакторинг метода `fromArray`

-   📚 Обновлена документация

---

## 1.0.0

-   🎉 Полностью рабочий код

-   📚 Завершена документация

---

## 0.0.1

-   🎉 Начальный коммит

---
